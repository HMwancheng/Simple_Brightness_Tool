...
namespace SimpleBrightness
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            using (Mutex mutex = new Mutex(false, "Global\\" + "HMSimpleBrightness_v33_DetailPolish"))
            {
        ...
        public BrightnessForm(List<MonitorInfo> monitors, AppConfig config, MyCustomApplicationContext context, ContextMenuStrip menu) {
            ...
            foreach (var m in visibleMonitors) {
                ...
                if (m.Type == MonitorType.DDC) { 
                    ...
                    // 修复：传入配置的电源模式
                    btnPower.MouseDown += (s, e) => { Task.Run(() => BrightnessController.SetPowerState(m, e.Button == MouseButtons.Right, _config.UseSoftwarePower)); }; 
                    // 修复：添加 ToolTip
