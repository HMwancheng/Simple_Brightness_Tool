...
namespace SimpleBrightness
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            // 互斥体防止多开
            using (Mutex mutex = new Mutex(false, "Global\\" + "HMSimpleBrightness_v28_Final_Complete"))
            {
    ...
    public class MyCustomApplicationContext : ApplicationContext
    {
        ...
        private void OnGlobalMouseWheel(object? sender, MouseEventArgs e)
        {
            ...
            bool isActive = (DateTime.Now - _lastIconHoverTime).TotalSeconds < 1.0; // 冷却时间

            if (isOverTray && isActive)
            {
                _lastIconHoverTime = DateTime.Now; // 持续操作保活
        ...
        private void ShowBrightnessWindow()
        {
            ...
            foreach (var m in monitors)
            {
                brightnessWindow.UpdateSlider(m.UniqueId, m.LastBrightness);
            }
        }
    ...
    // ================== 3. UI Classes (BrightnessForm, SettingsForm, etc) ==================
    ...
    public class BrightnessForm : Form
    {
        ...
        public BrightnessForm(List<MonitorInfo> monitors, AppConfig config, MyCustomApplicationContext context, ContextMenuStrip menu)
        {
            ...
            this.Deactivate += (s, e) =>
            {
                if (Application.OpenForms.OfType<CurveEditorForm>().Any() || Application.OpenForms.OfType<SettingsForm>().Any() || Application.OpenForms.OfType<InputBox>().Any() || Application.OpenForms.OfType<HelpForm>().Any()) return;
...
您可以将本文件夹移动到任何位置（便携模式）。"
    ...
    public class MouseHook
    {
        ...
        public void Install() { _hookID = SetWindowsHookEx(14, _proc, GetModuleHandle(System.Diagnostics.Process.GetCurrentProcess().MainModule?.ModuleName ?? "user32"), 0); }
