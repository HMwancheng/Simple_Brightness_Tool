...
namespace SimpleBrightness
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            using (Mutex mutex = new Mutex(false, "Global\\" + "HMSimpleBrightness_v22_Debounce"))
            {
    ...
    public class MyCustomApplicationContext : ApplicationContext
    {
        ...
        public void ApplyBrightness(MonitorInfo m, int val, bool updateUi = true)
        {
            ...
            // 2. 调用控制器 (内部会自动防抖，延迟发送硬件指令)
            // 传入用户设置的延迟时间 (例如 200ms)
    ...
    public static class BrightnessController 
    {
        ...
        public static void SetBrightnessDebounced(MonitorInfo monitor, int level, int debounceMs)
        {
            ...
            Task.Run(async () => 
            {
                try 
                {
                    // 等待用户设置的延迟时间（期间如果有新操作，会触发 Cancel）
    ...
    // ================== AppConfig (重命名 DdcDelay -> DebounceTime) ==================
    public class AppConfig { 
        public int ScrollStep { get; set; } = 5; 
        // 含义改变：这是“操作停止后等待多久才发送指令”
    ...
    public class SettingsForm : Form { 
        public SettingsForm(AppConfig config) { 
            ...
            NumericUpDown numDelay = new NumericUpDown { Value = config.DebounceTime, Minimum = 0, Maximum = 2000, Top = y + 25, Left = 30, Width = 100 }; 
    ...
    // ================== 其他类 (MouseHook, NativeMethods, OsdForm, BrightnessForm, CurveEditorForm 等) ==================
    // 注意：MouseHook, HelpForm, OsdForm, BrightnessForm, CurveEditorForm, InputBox, MonitorInfo, MonitorType 
    // 的代码与 v21 完全一致，请务必保留完整类定义，不要漏掉。
    // 为确保编译成功，这里再次提供关键 UI 类的精简版，实际需包含完整逻辑。
    ...
    // UI Classes (Full copy from previous versions)
