...
using System.Drawing.Text; // 引入字体处理
using System.IO;
using System.Linq;
...
namespace SimpleBrightness
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            // 互斥体防止多开
            using (Mutex mutex = new Mutex(false, "Global\\" + "HMSimpleBrightness_v20_ConfigFixed"))
            {
    ...
    public class MyCustomApplicationContext : ApplicationContext
    {
        ...
        public MyCustomApplicationContext()
        {
            ...
            config.Save(); // 强制生成文件，确保用户能看到配置文件
        ...
        private void SaveAllSettings()
        {
            // 将内存中的亮度值写入配置
            foreach(var m in monitors) {
                config.SavedBrightness[m.UniqueId] = m.LastBrightness;
            }
            config.Save();
        }
    ...
    public static class IconDrawer { 
        public static Icon DrawNativeIcon() { 
            ...
            using (Bitmap bmp = new Bitmap(32, 32)) using (Graphics g = Graphics.FromImage(bmp)) { 
                ...
                // 2. 尝试使用 Segoe MDL2 Assets (Win10/11 系统图标字体)
                // 代码 \uE706 是 "Brightness" 图标
                Font iconFont = new Font("Segoe MDL2 Assets", 18, FontStyle.Regular);
                
                // 3. 绘制白色 (深色模式通用)
                // 微调位置 (Point) 以确保居中
                TextRenderer.DrawText(g, "\uE706", iconFont, new Point(-1, 0), Color.White);

                // 4. 如果系统没有这个字体（极少见），可以回退到手绘
                // 但 Win10+ 肯定有，这里直接返回
    ...
    public class AppConfig { 
        ...
        // 修复：使用 Application.StartupPath 确保在 exe 同级目录
    ...
    // ================== 以下保持 v19 逻辑 (UI自动布局 & 功能) ==================
    // 篇幅限制，以下类与 v19 完全一致，请保留：
    // MouseHook, BrightnessController, NativeMethods, 
    // BrightnessForm (FlowLayout), CurveEditorForm, InputBox, OsdForm, SettingsForm, MonitorInfo, MonitorType, HelpForm
    
    // (为确保编译通过，此处提供精简版完整类结构，实际使用时请确保包含这些类的完整代码)
    ...
    // BrightnessForm, CurveEditorForm 等 UI 类代码与 v19/v14 保持一致，为节省篇幅不重复粘贴，
    // 请确保将之前版本的 UI 类代码（包括 BrightnessForm, CurveEditorForm, InputBox, OsdForm, SettingsForm, MonitorInfo, MonitorType, HelpForm）
    // 完整保留在 Program.cs 中。关键在于 AppConfig 的修改和 IconDrawer 的修改。
