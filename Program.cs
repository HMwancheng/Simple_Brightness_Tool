...
namespace SimpleBrightness
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            // 互斥体防止多开
            using (Mutex mutex = new Mutex(false, "Global\\" + "HMSimpleBrightness_v24_FullComplete"))
            {
    ...
    public class SettingsForm : Form { 
        public SettingsForm(AppConfig config) { 
            ...
            numStep.Value = Math.Clamp(config.ScrollStep, 1, 20); // 修复崩溃：先设置范围再赋值
            ...
            numDelay.Value = Math.Clamp(config.DebounceTime, 0, 2000); // 修复崩溃：先设置范围再赋值
