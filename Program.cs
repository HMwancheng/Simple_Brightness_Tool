...
namespace SimpleBrightness
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            using (Mutex mutex = new Mutex(false, "Global\\" + "HMSimpleBrightness_v21_FinalPolish"))
            {
    ...
    public static class BrightnessController { 
        // 静态锁，防止多线程同时发送 DDC 指令导致冲突
        private static object _ddcLock = new object();

        public static void SetBrightness(MonitorInfo monitor, int level, int delayMs) { 
            if (monitor.Type == MonitorType.WMI) { 
                ...
                lock (_ddcLock) {
                    ...
                    if (delayMs > 0) Thread.Sleep(delayMs); // 强制等待，防止指令堆积
                }
            } 
        } 
    ...
    public class SettingsForm : Form { 
        public SettingsForm(AppConfig config) { 
            this.Text = "设置"; this.Size = new Size(350, 480); // 增加高度
            ...
            btnOk.Click += (s, e) => { 
                ...
                config.DdcDelay = (int)numDelay.Value; // 保存延时
