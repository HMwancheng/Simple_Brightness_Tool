...
namespace SimpleBrightness
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            // 互斥体防止多开
            using (Mutex mutex = new Mutex(false, "Global\\" + "HMSimpleBrightness_v21_Full_Complete"))
            {
    ...
    public class MyCustomApplicationContext : ApplicationContext
    {
        ...
        public void ApplyBrightness(MonitorInfo m, int val, bool updateUi = true)
        {
            ...
            // 传入配置的 DDC 延时
            Task.Run(() => BrightnessController.SetBrightness(m, finalVal, config.DdcDelay));
    ...
    public static class IconDrawer { 
        public static Icon DrawNativeIcon() { 
            using (Bitmap bmp = new Bitmap(32, 32)) using (Graphics g = Graphics.FromImage(bmp)) { 
                ...
                // 修正：将 X 坐标设为 -4，向左平移，解决右侧被切、左侧留白问题
                // Y 坐标微调为 1，垂直居中
...
您可以将本文件夹移动到任何位置（便携模式）。"
    ...
    // ================== InputBox, OsdForm, SettingsForm, Config, DDC ==================
    ...
    public class SettingsForm : Form { 
        public SettingsForm(AppConfig config) { 
            ...
            Label lblDelay = new Label { Text = "DDC 通信延时 (ms):", Top = y, Left = 30, AutoSize = true, Font = new Font("Microsoft YaHei UI", 9) }; 
            NumericUpDown numDelay = new NumericUpDown { Value = config.DdcDelay, Minimum = 0, Maximum = 500, Top = y + 25, Left = 30, Width = 100 }; 
            ...
            btnOk.Click += (s, e) => { config.ScrollStep = (int)numStep.Value; config.DdcDelay = (int)numDelay.Value; SetAutoStart(chkAuto.Checked); this.Close(); }; 
    ...
    public class AppConfig { 
        ...
        public int DdcDelay { get; set; } = 30; 
    ...
    public static class BrightnessController { 
        private static object _ddcLock = new object();
        public static void SetBrightness(MonitorInfo monitor, int level, int delayMs) { if (monitor.Type == MonitorType.WMI) { try { var searcher = new ManagementObjectSearcher("root\\Wmi", "SELECT * FROM WmiMonitorBrightnessMethods"); foreach (ManagementObject m in searcher.Get()) m.InvokeMethod("WmiSetBrightness", new object[] { 1, level }); } catch {} } else if (monitor.Type == MonitorType.DDC) { lock(_ddcLock) { NativeMethods.SetVCPFeature(monitor.Handle, 0x10, (uint)level); if(delayMs > 0) Thread.Sleep(delayMs); } } } 
        public static void SetPowerState(MonitorInfo monitor, bool turnOn) { if (monitor.Type == MonitorType.DDC) { lock(_ddcLock) { NativeMethods.SetVCPFeature(monitor.Handle, 0xD6, turnOn ? 0x01u : 0x04u); } } } 
        public static int GetVCPBrightness(IntPtr hMonitor) { uint current = 50, max = 100; lock(_ddcLock) { if (NativeMethods.GetVCPFeatureAndVCPFeatureReply(hMonitor, 0x10, IntPtr.Zero, ref current, ref max)) return (int)current; } return -1; } 
    }
